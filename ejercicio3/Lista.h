...
public:
    ...
    Tipo* obtenerNodo(int posicion);

    //post: agrega un nuevo elemento a la lista
    void agregar(string nuevoElemento);

    void agregar_tipo(Tipo nuevoElemento);
...
template < typename Tipo >
void Lista<Tipo>::agregar_tipo(Tipo nuevoElemento) {
    Nodo<Tipo>* nuevoNodo = new Nodo<Tipo>(nuevoElemento);
    if(primero == nullptr){
        primero = nuevoNodo;
        ultimo = primero;
...
template<typename Tipo>
Tipo* Lista<Tipo>::obtenerNodo(int posicion) {
    int i = 0;
    Nodo<Tipo>* auxiliar = primero;
